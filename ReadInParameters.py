...
def  CompositeMaterials(surface_data,material):
    ...
    return  THcond[0:N_cells-1],densityC[0:N_cells-1],N_cells
...
def ReadInParameters(filename):
    ...
    [THcond,densityC,N_cells]=CompositeMaterials(surface_data,material)
    roof_upper=output_surface_parameters(THcond,material.loc[:,'HTCconv'].to_numpy(),densityC,0.05,surface_data.iloc[1,1],N_cells,upper_data_parameters_room[0])
    ##
    ...
    [THcond,densityC,N_cells]=CompositeMaterials(surface_data,material)
    wall_upper=output_surface_parameters(THcond,material.loc[:,'HTCconv'].to_numpy(),densityC,0.05,surface_data.iloc[1,1],N_cells,upper_data_parameters_room[0])
    ...
    windows_upper=output_surface_parameters(THcond,material.loc[:,'HTCconv'].to_numpy(),densityC,0.05,surface_data.iloc[1,1],N_cells_internal,upper_data_parameters_room[0])
...
#//////////// middle room /////////////////////
    ...
    [THcond,densityC,N_cells]=CompositeMaterials(surface_data,material)
    roof_middle=output_surface_parameters(THcond,material.loc[:,'HTCconv'].to_numpy(),densityC,0.05,surface_data.iloc[1,1],N_cells,upper_data_parameters_room[0])
    ...
    [THcond,densityC,N_cells]=CompositeMaterials(surface_data,material)
    wall_middle=output_surface_parameters(THcond,material.loc[:,'HTCconv'].to_numpy(),densityC,0.05,surface_data.iloc[1,1],N_cells,upper_data_parameters_room[0])
    ...
    THcond=np.full((1, N_cells_internal), material.loc[:,'Thermal conductivity'].to_numpy())
    densityC=np.full((1, N_cells_internal), material.loc[:,'DensityC'].to_numpy())
    windows_middle=output_surface_parameters(THcond,material.loc[:,'HTCconv'].to_numpy(),densityC,0.05,surface_data.iloc[1,1],N_cells_internal,middle_data_parameters_room[0])
    ...
    [THcond,densityC,N_cells]=CompositeMaterials(surface_data,material)
    roof_lower=output_surface_parameters(THcond,material.loc[:,'HTCconv'].to_numpy(),densityC,0.05,surface_data.iloc[1,1],N_cells,upper_data_parameters_room[0])
    ...
    [THcond,densityC,N_cells]=CompositeMaterials(surface_data,material)
    wall_lower=output_surface_parameters(THcond,material.loc[:,'HTCconv'].to_numpy(),densityC,0.05,surface_data.iloc[1,1],N_cells,upper_data_parameters_room[0])
    ...
    floor=output_surface_parameters(THcond,material.loc[:,'HTCconv'].to_numpy(),densityC,0.05,surface_data.iloc[1,1],N_cells,upper_data_parameters_room[0])
    ...
    THcond=np.full((1, N_cells_internal), material.loc[:,'Thermal conductivity'].to_numpy())
    densityC=np.full((1, N_cells_internal), material.loc[:,'DensityC'].to_numpy())
    windows_lower=output_surface_parameters(THcond,material.loc[:,'HTCconv'].to_numpy(),densityC,0.05,surface_data.iloc[1,1],N_cells_internal,upper_data_parameters_room[0])
